...
from densenet_components import Transition, make_dense_block
from errors import InvalidInputShapeError, InvalidModelClassCountError, MissingNetworkError
...
class MHeads(PytorchModelBase):
    def __init__(self, name=None, description=None):
        ...
        self.__backbone = None
        self.__input_shape = None
        self.__num_classes = None
        self.__head_depth = None
        self.__num_heads = None
        self.__mhead_random = None
        self.__mhead_eps = None
        self.__learning_rate = None
        self.__dropout_rate = None
        self.__layers_per_block = None
        self.__growth_rate = None
        self.__bottleneck = None
        self.__reduction = None
        self.__bottleneck_rate = None
        self.__pool_initial = None
        self.__pool_final = None
        self.__valid_padding = None
        self.__optimizer_type = None
        self.__optimizer_mom = None
        self.__optimizer_wd = None
        self.__multi_gpu = None
    ...
    def configure(self, input_shape, num_classes, backbone, optimizer_type, optimizer_mom, optimizer_wd, head_depth,
        ...
        if len(input_shape) != 3 or min(input_shape) <= 0:
            raise InvalidInputShapeError(input_shape)

        if num_classes <= 1:
            raise InvalidModelClassCountError(num_classes)
    ...
    def _setcustomdata(self, data_maps):
        """Set custom data from loaded model."""
    ...
    def _networkdefinition(self):
        ...
        Returns:
            nn.Module: The network architecture
        """
        if self.__backbone == 'densenet':
            return DenseNetMHeads(
                input_shape=self.__input_shape,
                pool_initial=self.__pool_initial,
                pool_final=self.__pool_final,
                layers_per_block=self.__layers_per_block,
    ...
    def build(self):
        ...
        self._model_instance.to(self.device)

        if self.__multi_gpu and torch.cuda.device_count() > 1:
            self._model_instance = nn.DataParallel(self._model_instance)

        if self.__optimizer_type == 'adam':
            self._optimizer = torch.optim.Adam(
                self._model_instance.parameters(),
                lr=self.__learning_rate,
                weight_decay=self.__optimizer_wd
            )
        elif self.__optimizer_type == 'sgd':
            self._optimizer = torch.optim.SGD(
                self._model_instance.parameters(),
                lr=self.__learning_rate,
                momentum=self.__optimizer_mom,
                nesterov=True,
                weight_decay=self.__optimizer_wd
            )
        else:
    ...
    def reset_head_report(self, inference=False):
        if inference:
            ...
            self.head_report = {
                'training': Counter({i: 0 for i in range(self.__num_heads)}),
    ...
    def predict(self, x, y=None, sample_weight=None, class_weight=None, *args, **kwargs):
        ...
        if self._output_offset is None:
            self._output_offset, _, _ = self.getreconstructioninformation((x.shape[2], x.shape[3]))
        ...
        if sample_weight is not None:
            sample_weight = self._matchdatatonetwork(sample_weight)
    ...
    def getreconstructioninformation(self, input_shape=None):
        ...
        Args:
            input_shape (tuple): Input shape (width, height)

        Returns:
            tuple: (lost pixels, downsample factor, interpolation factor)
